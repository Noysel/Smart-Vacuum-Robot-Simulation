...
public class LiDarService extends MicroService {
     ...
     * and s(ets up the necessary callbacks for processing data.
     */
    @Override
    protected void initialize() {
        subscribeBroadcast(TickBroadcast.class, (Callback<TickBroadcast>) tickBroadcast -> {
            timeTick = tickBroadcast.getTime();
            List<DetectObjectEvent> eventList = liDar.getTimedEvents(timeTick);
            if (eventList != null){
                for (DetectObjectEvent event : eventList) {
                    // COMPLETE
                    complete(event, true);
                }
        ...
        subscribeEvent(DetectObjectEvent.class, detectetObjEve -> {
            List<TrackedObject> listTracked = liDar.interval(timeTick, detectetObjEve);
            if (listTracked != null){
                Future<LandMark> future = sendEvent(new TrackedObjectEvent(listTracked));
                complete(detectetObjEve, true);
